...
void lcd_draw_pictrue(int x_start, int y_start, int x_end, int y_end, const unsigned char *gImage)
{
    ...
    size_t copied = 0;
    while (copied < pixels_byte_size) {
        size_t copy_size = (pixels_byte_size - copied > chunk_size) ? chunk_size : (pixels_byte_size - copied);
        memcpy((uint8_t*)pixels + copied, gImage + copied, copy_size);
        copied += copy_size;
    }
    
    esp_lcd_panel_draw_bitmap(panel_handle, x_start, y_start, x_end, y_end, pixels); // 显示整张图片数据
